...
struct IRK16_SIMD_Cache{floatType,fType,pType}
    odef::fType # function defining the ODE system
    p::pType # parameters and so
    h::floatType
    hb::Vec{8,floatType}
    hc::Vec{8,floatType}
    mu::Vec8Vector{floatType}
    nu::Vec8Vector{floatType}
    U::Vec8Vector{floatType}
    Uz::Vec8Vector{floatType}
    L::Vec8Vector{floatType}
    F::Vec8Vector{floatType}
    Dmin::Vector{floatType}
    itermax::Int64
...
function  IRK16_SIMD(u0::Vector{floatType}, t0::floatType, tf::floatType, n, m, f, p;  initial_interp=true, rbits=3,  itermax=100, trace=false) where {floatType <: Union{Float32,Float64}}


    rdigits = convert(floatType, 2^rbits)
    step_number = Array{Int64,0}(undef)
    step_number[] = 0
    h = (tf-t0)/(n*m)
    (hb_, hc_, mu_, nu_) = IRK16Coefficients(h)
    s = length(hb_)
    dim = length(u0)
    hc = vload(Vec{8,floatType}, hc_, 1)
    hb = vload(Vec{8,floatType}, hb_, 1)
    nu=Vec8Vector(zeros(floatType,8,8))
    mu=Vec8Vector(zeros(floatType,8,8))
    for i in 1:s 
       nu[i] = vload(Vec{8,floatType}, nu_[i,:], 1)  # iraulia
       mu[i] = vload(Vec{8,floatType}, mu_[i,:], 1)  # iraulia
    end
    utype = Vector{floatType}
    ttype = floatType
    U = Vec8Vector(zeros(floatType, s, dim))
    Uz = Vec8Vector(zeros(floatType, s, dim))
    L = Vec8Vector(zeros(floatType, s, dim))
    F = Vec8Vector(zeros(floatType, s, dim))
    Dmin = zero(u0) 
    ej=zero(u0)
    
    uu = Array{Vector{floatType}}(undef, n+1)
    tt = Array{ttype}(undef, n+1)
    irk16cache = IRK16_SIMD_Cache(f,p,h,hb,hc,mu,nu,U,Uz,L,F,Dmin,itermax,
                                  step_number,initial_interp,rdigits, trace)
    iters = zeros(n+1)
    uu[1] = copy(u0)
    tt[1] = t0
    tj = [t0, zero(t0)]
    ...
    for j in 1:n
        for i in 1:m
           irk16cache.step_number[] += 1 
           j_iter = IRK16step!(tj,uj,ej,irk16cache)
           iters[j+1] += j_iter
        end
        iters[j+1] /= m  # average number of iterations per step
        uu[j+1] = uj + ej
        tt[j+1] = tj[1] + tj[2]
    end
...
function IRK16step!(ttj,uj,ej,irk16cache::IRK16_SIMD_Cache{floatType,fType,pType}) where {floatType,fType,pType}
       ...
       L = irk16cache.L
       F = irk16cache.F
       Dmin = irk16cache.Dmin
       step_number = irk16cache.step_number[]
       initial_interp = irk16cache.initial_interp
       rdig = irk16cache.rdigits
       trace = irk16cache.trace
       s = length(hb)
       dim = length(uj)
       itermax = irk16cache.itermax
       tj = ttj[1]
       te = ttj[2]
       ...
       if initial_interp
         for k in indices   
            ...
            v7=sum(nu[7]*L[k])
            v8=sum(nu[8]*L[k])
            U[k]=uj[k]+(ej[k]+Vec(v1,v2,v3,v4,v5,v6,v7,v8))
        end
       else
          for k in indices
             uej = uj[k] + ej[k]
    ...
    @inbounds while (j_iter<itermax && iter)
            j_iter += 1
           ...
           for k in indices
            ...
            if (iter==false && D0<dim && plusIt)  # if DY => Dmin[is][k] for all k, one (and only one) additional iteration is performed
                iter=true
                plusIt=false
            else
                plusIt=true
...
#            for is in 1:s
#	         e0 += muladd(F.data[is,k], hb[is], -L.data[is,k])
#            end
            e0+=sum(F[k]*hb-L[k])
