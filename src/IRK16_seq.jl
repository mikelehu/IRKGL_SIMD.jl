...
struct IRK16Cache{uType,tType,fType,pType}
...
function  IRK16(u0::utype, t0::ttype, tf::ttype, n, m, f, p;  initial_interp=true, rbits=3,  itermax=100, trace=false) where {utype, ttype}
    rdigits = convert(ttype, 2^rbits)
    step_number = Array{Int64,0}(undef)
    step_number[] = 0
    h = (tf-t0)/(n*m)
    (hb, hc, mu, nu) = IRK16Coefficients(h)
    s = length(hb)
    sm = s*m
    ...
    irk16cache = IRK16Cache(f,p,h,hb,hc,mu,nu,U,Uz,L,F,Dmin,itermax,step_number,initial_interp,rdigits, trace)
    iters = zeros(n+1)
    uu[1] = copy(u0)
    tt[1] = t0
    tj = [t0, zero(t0)]
    ...
    for j in 1:n
        for i in 1:m
           ...
           j_eval = IRK16step!(tj,uj,ej,irk16cache)
           iters[j+1] += j_eval
        end
...
function IRK16step!(ttj,uj,ej,irk16cache::IRK16Cache)
       f = irk16cache.odef
       p = irk16cache.p
       h = irk16cache.h 
       hb = irk16cache.hb 
...
#            @. L[is] =  hb[is]*F[is] # Hau behekoa bezain azkarra dala dirudi
#             Dmin[is] .= Inf
            for k in indices
                L[is][k] = hb[is]*F[is][k]
                Dmin[is][k] = Inf
    ...
    @inbounds while (j_eval<sitermax && iter)
            ...
            =#
                for k in indices
                    Uz[is][k] = U[is][k]
                    U[is][k] = uj[k] + (ej[k] + mu[is,1]*L[1][k] + mu[is,2]*L[2][k] + mu[is,3]*L[3][k] + mu[is,4]*L[4][k] +
                             mu[is,5]*L[5][k] + mu[is,6]*L[6][k] + mu[is,7]*L[7][k] + mu[is,8]*L[8][k] )
                end
            ...
            for is in 1:s
                ...
                eval=false
                for k in indices
                            DY=abs(Rdigits(U[is][k],rdig)-Rdigits(Uz[is][k],rdig))  
#                            if trace
#                                DY > DYmax ? (ismax=is; kmx=k; DYmax=DY; println((kmx, DYmax))) : nothing 
#                            end
                
                            if DY>0.
                               # eval=true  # Lehen hemen zegoen, baina ez da egokia rdig=0 ez bada
                               if DY< Dmin[is][k]
                                  Dmin[is][k]=DY
                                  iter=true
                               end
               ...
               if eval
                    if trace
                        println("is=$is")
                        println(NbodyLog2RelErrors(U[is], Uz[is],rdig))
                    end
                    f(F[is], U[is], p,  tj  + hc[is])
...
#                   @. L[is] = hb[is]*F[is]
            ...
            if (iter==false && D0<elems && plusIt)  # if DY > Dmin[is][k] for all k, one (and only one) additional iteration is performed
                iter=true
                plusIt=false
            else
                plusIt=true
